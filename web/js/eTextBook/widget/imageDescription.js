var eTextBookWidgetImageDescription = eTextBookWidget.extend({
  defaults: {
    slug: "image-description", title: "Описание картинки", templateName: 'imageDescription'
  },
  ...
  activate: function () {
    var $this = this;
    this.editCont.find('image-description images')
      .append('<edit-element class="add-image">' +
        '<span class="glyphicon glyphicon-picture"></span>' +
        '<p class="help-text">Добавить картинку</p>' +
        '</edit-element>');
    this.editCont.find('image-description images item image-view')
      .append('<edit-element class="glyphicon glyphicon-remove remove"></edit-element>');
    this.editCont.find('image-description images item image-title')
      .append('<edit-element><input type="text" /></edit-element>');
  ...
  viewActivate: function () {
    ...
    for (i = 0; i < images.length; i++) {
      ...
      this.descs.push({
        answer: pos, text: image.find('image-title view-element').html()
      });
    }
